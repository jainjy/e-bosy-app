...
import {
  PlayCircleIcon,
  DocumentTextIcon, // Used for PDFs
  PlayIcon, // Used for video
...
const CourseDetailsPage = () => {
  ...
  const getContentTypeIcon = (contentType) => {
    switch (contentType.toLowerCase()) {
      case 'pdfs': // Changed from 'pdf' to 'pdfs' to match backend
      case 'document':
        return <DocumentTextIcon className="h-5 w-5 text-red-500 mr-2" />;
      case 'video':
      case 'external_video_url': // Include external video links for icon
  ...
  // Filter sections to only show those that have actual lessons
  const sectionsWithLessons = course.sections?.filter(section =>
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow-md p-6 animate-slide-in-left">
          ...
          {sectionsWithLessons.map((section, index) => {
            // Filter lessons for this specific section
            let lessonsInSection = lessons.filter(lesson => lesson.sectionTitle === section.title);

            // Sort lessons: Videos first, then PDFs, then other content types
            lessonsInSection.sort((a, b) => {
              ...
              if (typeA === 'video' && typeB !== 'video') {
                return -1; // 'a' (video) comes before 'b'
              }
              if (typeA !== 'video' && typeB === 'video') {
                return 1; // 'b' (video) comes before 'a'
              }

              // Then, prioritize PDFs (if both are not videos)
              // If 'a' is a PDF and 'b' is neither video nor PDF, 'a' comes after 'b'
              if (typeA === 'pdfs' && typeB !== 'pdfs') {
                return 1;
              }
              // If 'b' is a PDF and 'a' is neither video nor PDF, 'b' comes after 'a'
              if (typeA !== 'pdfs' && typeB === 'pdfs') {
                return -1;
              }

              // For other content types or if types are the same, sort alphabetically by title
              return a.title.localeCompare(b.title);
            });

            return (
              <div key={index} className="mb-6 border border-gray-200 rounded-lg overflow-hidden transition-all duration-300 hover:shadow-lg">
                <div className="bg-gray-100 p-4 font-semibold text-lg">
                  <span>{section.title}</span>
                </div>
                <ul className="divide-y divide-gray-200">
                  {lessonsInSection.map(lesson => {
                    const isLessonAccessible = isEnrolled && (!lesson.isSubscriberOnly || user?.IsSubscribed);
                    const canDownload = isLessonAccessible &&
                                        lesson.content &&
                                        (lesson.contentType.toLowerCase() === 'video' ||
                                         lesson.contentType.toLowerCase() === 'pdfs'); // Corrected to 'pdfs'

                    return (
                      <li key={lesson.lessonId} className="p-4 flex items-center justify-between group transition-colors duration-200 hover:bg-gray-50">
                        <Link
                          to={isLessonAccessible ? `/course/${course.courseId}/lesson/${lesson.lessonId}` : '#'}
                        ...
                        {canDownload && (
                          <a
                            ...
                            onClick={(e) => {
                                // Optional: Add a check here if needed, though 'canDownload' should prevent reaching this if conditions aren't met
                                if (!isLessonAccessible) {
                                    e.preventDefault();
                                    toast.error("Vous n'êtes pas autorisé à télécharger ce contenu.");
                                }
                            }}
                          >
          ...
          <div className="bg-white rounded-lg shadow-md p-6 transform transition-transform duration-300 hover:scale-[1.02]">
            ...
            <div className="text-gray-700 space-y-3">
              <p className="flex items-center">
                ...
                Dernière mise à jour: {new Date(course.updatedAt).toLocaleDateString('fr-FR')}
              </p>
              <p className="flex items-center">
              ...
              {(course.averageRating !== undefined && course.totalReviews !== undefined) && (
                <p className="flex items-center">
